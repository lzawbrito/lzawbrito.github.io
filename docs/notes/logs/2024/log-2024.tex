...
\begin{itemize}[nosep]
\item \textit{What is a spin connection?} It is the analogue of the Levi-Civita 
	(or affine) connection for spinors. From a technical point of view, it is 
	the pullback to the spinor group $ \text{Spin}(4) $ of the Levi-Civita 
	connection (1-form) induced by the map $ \varphi: \text{Spin}(4) \to \text{SO}(4) $. 
\item Why frame the Wilson line?
\item Why $ L_- $ and $ \Delta $ nonsingular iff de Rham cohomology trivial for 
	all dimensions?
\end{itemize}
...
\begin{align*}
	(F+ d\alpha)\wedge (F+ d\alpha)
		&= F\wedge F + \cancel{d\alpha \wedge F} + \cancel{F\wedge d\alpha}
			+ d\alpha\wedge d\alpha\\
		&= F\wedge F + d^2\alpha \wedge \alpha 
			\pm \underbrace{\cancel{d(\alpha \wedge d\alpha)}}_{\text{no }\partial X}
\end{align*}
...
\parhead{1st S-W class} The first Stiefel-Whitney class is defined as the determinant of the transition 
maps on the charts of the manifold. Consider two charts $ U_i $ and $ U_j $
related by the transition map $ t_{ij} $, itself a member of the structure 
group $ \text{SO}(m =\dim M) $. ($ i $, $ j $ here do not index the entries 
of the matrix but instead label the charts.) We define a $ 1 $-cochain 
with coefficients in $ \mathbb{Z}_2 $ by the determinant of the transition 
function: 
\begin{equation*}
...
Now, the Stiefel-Whitney class is trivial---all functions in $ [f] $ 
...
\parhead{2nd S-W class} The second Stiefel-Whitney class measures a slightly different obstruction: instead 
...
Calculating the determinant is at best an $ O(n^3) $ task

TODO explain what bug I had 
